...
STATISTIC(NumStack,      "Number of stacks");
...
//      AU.addPreservedID(LoopSimplifyID);      // 583 - commented out
  ...
  private:
    ...
    void alloc_wrapper(Instruction &I);
    AllocaInst *flag_wrapper(Instruction *I);
    void reload(Instruction *I, Instruction *consumer);
    bool consumer_detect(Instruction *I);
    Instruction *consumer_flag(Instruction *I, AllocaInst *flag);
    void redoBB_split(Instruction *split_loc, Instruction *Ibegin, Instruction *Iend, AllocaInst *flag);
...
void SLICM::HoistRegion(DomTreeNode *N) {
  ...
  list<Instruction *>::iterator hoisted_itr;
  hoisted_itr = hoistedTop.begin();
  Instruction *redoBB_begin, *redoBB_end;
  Instruction *I;
  unsigned int redoBB_counter = 0;

  for (hoisted_itr = hoistedTop.begin(); 
      hoisted_itr != hoistedTop.end(); hoisted_itr++) 
  {
    I = dyn_cast<Instruction>(*hoisted_itr);
    errs() << "Hoisted >> " << *I << "\n";
    if (!redoBB_counter) 
      redoBB_begin = I;
    redoBB_counter++;
  ...
  if (!redoBB_counter) {
    /* errs() << "Something gotta be wrong...! [Terminating...]\n"; */
    return;
  }

  redoBB_end = I;
  /* errs() << "start: " << *redoBB_begin << "\n"; */
  /* errs() << "  end: " << *redoBB_end << "\n"; */

  list< pair<Instruction *, Instruction *> > redoBB_pair_list;
      ...
      hoisted_itr != hoistedTop.end(); hoisted_itr++) 
  {
    ...
    for (Value::use_iterator UI = I->use_begin(); UI != I->use_end(); UI++)
    {
      ...
      errs() << "Inst: " << *I << "\n";
      /* create flag only if **used** by other instructions */
      if (consumer->getParent() != I->getParent() && isa<StoreInst>(*consumer)) {
        consumer_found = true;
        ...
        reload(I, consumer);
        flag = flag_wrapper(I);
        flag_map[I] = flag; // retrieve by "end" redoBB
      /* } else { */ 
      /*     errs() << "\tConsumer in same block: " << *consumer << "\n"; */
      }
    }
    ...
    if (consumer_found) 
    {
      /* errs() << "start: " << *redoBB_begin << "\n"; */
      /* errs() << "  end: " << *I << "\n"; */
      pair<Instruction *, Instruction *> p(redoBB_begin, I);
      redoBB_pair_list.push_back(p);
    }
  }
  ...
  {
    Instruction *temp_begin = set_iter->first;
    Instruction *temp_end = set_iter->second;
    errs() << "start: " << *temp_begin << "\n";
    errs() << "  end: " << *temp_end << "\n";

    bool consumer_found = false;
    Instruction *split_loc;
    I = temp_begin;
    consumer_found = consumer_detect(I);
    if (consumer_found) {
      flag = flag_map[temp_end];
      split_loc = consumer_flag(I, flag);
...
void SLICM::redoBB_split(Instruction *split_loc, Instruction *Ibegin,
    Instruction *Iend, AllocaInst *flag)
{
  BasicBlock *home, *redo, *rest, *const_home, *BB;
  BasicBlock::iterator bbiter, redo_begin, rest_begin;

  BB = Ibegin->getParent();
  errs() << "starting inst: " << *Ibegin << "\n";
  errs() << "  ending inst: " << *Iend << "\n";

  home = split_loc->getParent();
  errs() << "original... : " << *home << "\n";
  const_home = home;
  errs() << "splitting inst: " << *split_loc << "\n";
  errs() << "splittig block: " << *home << "\n";

  IRBuilder<> flag_builder(Preheader);
  flag_builder.SetInsertPoint(split_loc);
  Value *cond = flag_builder.CreateLoad(flag);
  errs() << ">>>> Cond: " << *cond << "\n";
  ...
  Instruction *stack = NULL, *last_val = NULL, *last_compute_val = NULL;
  IRBuilder<> irbuilder(Preheader);
  irbuilder.SetInsertPoint(bbiter);
  Value *op0 = NULL, *op1 = NULL;
  ...
  for (BasicBlock::iterator II = BB->begin(), E = BB->end(); II != E; II++) {
    ...
    if (begin_found) {
      errs() << "\tcopying: " << I << "\n";
      if (last_val && (last_val == II)) {
        break;
      }
      op0 = II->getOperand(0);
      if (op0) {
        errs() << "op0     : " << *op0 << "\n";
        errs() << "op0 type: " << *op0->getType() << "\n";
        /* errs() << "op0 name: " << op0->getName() << "\n"; */
      }

      op1 = II->getOperand(II->getNumOperands() - 1);
      if (op1) { 
        errs() << "op1     : " << *op1 << "\n";
        errs() << "op1 type: " << *op1->getType() << "\n";
        errs() << "op1 name: " << op1->getName() << "\n";
        /* detect constants.... */
        if (op1->getName() != "") {
          op1->print(errs());
          errs() << "\n";
      ...
      { 
        /* errs() << "out stack is... " << *last_val << "\n"; */
        newInst->setOperand(0, last_val);
        if (last_compute_val && op1 && op1->getName() != "") { 
          errs() << "LastCmputeVal: " << *last_compute_val << "\n";
          newInst->setOperand(1, last_compute_val);
        }
      }
      errs() << "NewInst: " << *newInst << "\n";
      /* errs() << "new op0: " << *newInst->getOperand(0) << "\n"; */
  ...
  stack = stack_map[Iend];
  op0 = last_val->getOperand(0);
  errs() << "op0 top : " << *op0 << "\n";
  errs() << "op0 top type : " << *op0->getType() << "\n";

  op1 = last_val->getOperand(last_val->getNumOperands() - 1);
  errs() << "op1 top : " << *op1 << "\n";
  errs() << "op1 top type : " << *op1->getType() << "\n";
  errs() << "stack: " << *stack << "\n";

  StoreInst *st = irbuilder.CreateStore(last_val, stack);
  errs() << "store: " << *st << "\n";
...
AllocaInst *SLICM::flag_wrapper(Instruction *I)
{
  AllocaInst *flag;
  BasicBlock *parent = I->getParent();
  IRBuilder<> flag_builder(Preheader);
  flag_builder.SetInsertPoint(parent->begin());
  
  flag = flag_builder.CreateAlloca(Type::getInt1Ty(getGlobalContext()), 0, "flag");
  errs() << "\tflag: " << *flag << "\n";
...
Instruction *SLICM::consumer_flag(Instruction *I, AllocaInst *flag)
{
      ...
      itr != alias_map.end(); itr++) 
  {
    ...
    Value *Op = I->getOperand(I->getNumOperands() - 1);
        ...
        itr2++) 
    {
      ...
      errs() << "A>> " << *aliasI << "\n";
      ICmpInst *ICMP;
      if (NumStack > 1) // JOSEPH
         ICMP = new ICmpInst(aliasI->getParent()->getTerminator(),
             ICmpInst::ICMP_UGE, aliasI->getOperand(1), newI);
      else 
         ICMP = new ICmpInst(aliasI->getParent()->getTerminator(),
...
bool SLICM::consumer_detect(Instruction *I) 
{
  bool ret = false;
  Value *Op = I->getOperand(I->getNumOperands() - 1);
  errs() << "ld's last Op: " << *Op << "\n";
...
void SLICM::reload(Instruction *I, Instruction *consumer) 
{
  IRBuilder<> irbuilder(Preheader);
  irbuilder.SetInsertPoint(consumer->getParent()->begin());
  Instruction *stack = stack_map[I];

  LoadInst *ld = irbuilder.CreateLoad(stack, "temp");
  errs() << "\tld: " << *ld << "\n";

  consumer->setOperand(0, ld);
  errs() << "\tst: " << *consumer << "\n";

  return;
}

void SLICM::alloc_wrapper(Instruction &I)
{
  IRBuilder<> irbuilder(Preheader);
  irbuilder.SetInsertPoint(I.getParent()->begin());

  Twine *name = new Twine(100+(++NumStack));
  AllocaInst *stack =
    irbuilder.CreateAlloca(Type::getInt32Ty(getGlobalContext()), 0, *name); 
  errs() << "\tAlloca " << *stack << "\n";
